...
export default function AdminChatPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Admin can list all projects via admin route
  ...
  return (
    <main className="mx-auto w-full max-w-[1100px] px-6 py-8 text-textPrimary">
      ...
      {projectId ? (
        <ChatBox projectId={projectId} role="admin" token={token} />
      ) : (
        <div className="bg-bgSectionDark border border-borderSoft rounded-xl shadow-card p-12 text-center">
          <Empty description="No projects available. Create or assign a project to start chatting." />
        </div>
      )}
    </main>
  );
